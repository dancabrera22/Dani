...
function findValue(line, from, opts) {
  ...
  while ((m = NUM_RE.exec(seg)) !== null) {
    ...
    // pula números que são parte de data/hora (12/08/26, 07:30, "24 horas")
    if (/^\s*[/:]\d/.test(after) || prevCh === '/' || prevCh === ':') {
      continue;
    }
    if (/^\s*h(oras?)?\b/.test(after)) {
      continue;
    }
...
const GENERIC_SKIP =
  /(hemograma|eritrograma|leucograma|contagem diferencial|gasometria|coagulograma|plaquetas|resultado|urina\b|\beas\b|observa|\bnota\b|adendo|assinatura|conferido|repetido|confirmado|automatizado|tabela|referencia|\bindice\b|conclusao|comentario|metodo|material|\bamostra\b|reagente)/;
...
const UNIT_AFTER =
  /^\s*(%|μ|µ|u[gil]?\b|ui\b|uui\b|mg\b|mg\/|g\/|g\b|d?l\b|meq|mmol|mcg|ng\b|ng\/|pg\b|pg\/|fl\b|mil\b|milh|mm3|mm³|seg\b|\/|x\s?10|k\/|m?osm)/;

// Cópias de PDF podem perder as quebras de linha ("paredão" de texto em uma
// linha só) — o parser é linha-a-linha e não reconheceria nada. Detecta e
// re-segmenta pelos marcos estruturais do laudo.
export function resegment(text) {
  const lines = text.split(/\r?\n/).filter((l) => l.trim());
  const avg = text.length / Math.max(1, lines.length);
  if (avg <= 220) return { text, resegmented: false };
  let t = text;
  t = t
    .replace(/(?=Solicit[cç]?a[cç][ãa]o\s*:)/g, '\n')
    .replace(/(?=Liberado em\s*:)/gi, '\n')
    .replace(/(?=Impresso em\s*:)/gi, '\n')
    .replace(/(?=Laborat[óo]rio\s*:)/g, '\n')
    .replace(/(?=Solicitante\s*:)/g, '\n')
    .replace(/(?=M[ée]todo\s*:)/g, '\n')
    .replace(/(?=RESULTADO)/g, '\n')
    .replace(/(?=Resultado\s*:)/g, '\n')
    .replace(/(?=Valor\s*:)/g, '\n')
    .replace(/(?=[ÍI]ndice\s*:)/g, '\n')
    .replace(/(?=Data de Coleta)/gi, '\n')
    .replace(/(?=Coletado em)/gi, '\n')
    .replace(/(?=Paciente\s*:)/g, '\n')
    .replace(/(?=Cliente\s*:)/g, '\n')
    .replace(/(?=Nome\s*:)/g, '\n')
    .replace(/(?=NOTA\s*\d?\s*:)/g, '\n')
    .replace(/(?=Nota\s*:)/g, '\n')
    .replace(/(?=Respons[áa]veis T[ée]cnicos)/g, '\n')
    .replace(/(?=Assinado eletronicamente)/g, '\n')
    .replace(/(?=Observa[çc][ãa]o)/g, '\n')
    .replace(/(?=Adendo)/g, '\n')
    // registro + nome do paciente ("0006133032K ERICK ...")
    .replace(/(?=\b\d{8,12}[A-Z]?\s+[A-ZÀ-Ü]{2,}\s+[A-ZÀ-Ü]{2,})/g, '\n');
  // título de exame em CAPS emendado após minúscula/dígito/pontuação
  t = t.replace(
    /([a-zà-ÿ0-9,.%)\]]) (?=[A-ZÀ-Ü][A-ZÀ-Ü ()/,.-]{9,}(?: |$))/g,
    '$1\n'
  );
  // títulos terminados no material: quebra ANTES do título e DEPOIS do sufixo
  // ("...COMPENSADO CREATININA (SORO) 2,78..." -> título e valor em linhas)
  t = t
    .replace(/ (?=[A-ZÀ-Ü][A-ZÀ-Ü0-9/ -]*\s*\((?:SORO|URINA[^)]*|PLASMA)\))/g, '\n')
    .replace(/(\((?:SORO|URINA[^)]*|PLASMA)\))\s+/g, '$1\n')
    .replace(/ (?=[A-ZÀ-Ü][A-ZÀ-Ü0-9/ -]*,\s?SANGUE)/g, '\n')
    .replace(/(,\s?SANGUE(?:\s+TOTAL)?)\s+/g, '$1\n')
    .replace(/(?=GASOMETRIA)/g, '\n');
  return { text: t, resegmented: true };
}

export function parseReport(text, opts = {}) {
  const warnings = [];
  const reseg = resegment(text);
  if (reseg.resegmented) {
    text = reseg.text;
    warnings.push(
      'O texto veio sem quebras de linha (cópia de PDF) — as linhas foram reconstruídas automaticamente. Confira com atenção redobrada.'
    );
  }
  ...
  let ctx24 = false; // bloco de urina de 24 horas (marca componentes .h24)
  ...
  let lastGeneric = null; // {r, ttl} — p/ anexar "Valor:/Índice:" ao qualitativo
  ...
  function urineLine(nline, rline) {
    ...
    for (const a of COMPILED) {
      if (!['PC', 'MC', 'CACR', 'ClCr', 'U-CrQ', 'U-AlbQ', 'U-ProtQ', 'U-CaQ', 'U-NaQ', 'U-KQ', 'U-ClQ', 'U-PQ', 'U-OsmQ', 'U-VolQ', 'U-UrQ', 'U-MgQ', 'U-AUQ', 'U-GlicQ'].includes(a.id)) continue;
  ...
  function pushResult(a, v, dt, qual = '') {
    ...
    if (a.hidden && a.id !== 'U-VolQ') value *= urineUnitFactor(v.after);
    // volume urinário: litros -> mL
    if (a.id === 'U-VolQ' && /^\s*l\b/.test((v.after || '').trim())) value *= 1000;
    ...
    // plausibilidade: creatinina > 30 e fósforo > 20 mg/dL não existem
    // (linha de referência, "antígeno P24" etc.)
    if (a.id === 'Cr' && value > 30) return false;
    if (a.id === 'P' && value > 20) return false;
    let id = a.id;
    ...
    if ((a.hidden || a.id === 'ClCr') && ctx24) r.h24 = true;
    if (opts.debug) r.src = srcLine.trim().slice(0, 100);
    results.push(r);
    return true;
  }
  ...
  for (const rawLine of rawLines) {
    ...
    // coleta de 24 horas: marca o bloco (componentes ganham .h24)
    if (/(urina\s+(de\s+)?24|24\s*horas|\b24\s*h\b|\b24h\b)/.test(norm)) ctx24 = true;
    ...
    if (ADMIN_SKIP.test(norm)) {
      ...
      // e a pendência de relação/componente urinário (nunca cruzam blocos)
      if (/solicit/.test(norm)) {
        urContext = null;
        pendingRatio = null;
        ctx24 = false;
      }
    ...
    if (URINE_SECTION.test(norm)) {
      ...
      pending = null;
      pendingGeneric = null;
      // título de DOSAGEM urinária ("FOSFORO (URINA ISOLADA)", relações):
      // não é seção de EAS — o valor é consumido no fluxo normal
      if (pendingRatio) inUrine = false;
      continue;
    }
    ...
    // (REF_WRAP não bloqueia aqui: a cauda "Mudança de metodologia..." pode
    // vir COLADA na linha do resultado qualitativo — o extrator corta antes)
    if (pendingGeneric && !RANGE_LINE.test(line)) {
      ...
      let rest = rm ? t.slice(rm[0].length).trim() : t;
      let restNorm = rm ? nt.slice(rm[0].length).trim() : nt;
      // nome do exame REPETIDO na linha do resultado ("SARAMPO IgM Não Reagente")
      const pname = normalize(pendingGeneric.name).replace(/\s+/g, ' ').trim();
      if (!rm && pname && restNorm.startsWith(pname + ' ')) {
        rest = rest.slice(pname.length).trim();
        restNorm = restNorm.slice(pname.length).trim();
      }
      let val = null;
      if (/^[<>]?\s*-?\d/.test(restNorm) && (rm || VALUE_LINE.test(nt)) && !REF_WRAP.test(line)) {
        ...
        // qualitativo no INÍCIO: se a referência vem colada sem separador
        // ("Reagente <16,5 UA/mL Reagente: ..."), fica só o qualificador
        const lead = restNorm.match(
          /^((n[aã]o\s+)?(reagente|detectad[oa]|positiv[oa]|negativ[oa]|indeterminad[oa]|inconclusiv[oa]|ausente|presente))\b/
        );
        const afterLead = lead ? restNorm.slice(lead[0].length).trim() : '';
        if (lead && afterLead.startsWith(':')) {
          // legenda de referência ("Reagente: >= 1,00 ..."), nunca resultado
          val = null;
        } else if (lead && !afterLead.startsWith(',')) {
          val = rest.slice(0, lead[0].length);
        } else {
          ...
          // frases de nota, tabelas e fragmentos de título não são resultado
          const words = val ? val.split(/\s+/).length : 0;
          if (
            val &&
            (words > 6 ||
              val.includes(':') ||
              /\d{2}\/\d{2}\/\d{2,4}/.test(val) || // datas ("a partir de 18/10/2022")
              /\b(partir|pesquisa|antigeno|ant[íi]geno|anticorpos)\b/i.test(val) ||
              REF_WRAP.test(normalize(val)))
          ) {
            val = null;
          }
        }
      ...
      if (val) {
        const pushed = pushGeneric(pendingGeneric.name, val);
        // qualitativo pode ganhar o número da linha "Valor:/Índice:" seguinte
        if (pushed && !/\d/.test(val)) lastGeneric = { r: pushed, ttl: 5 };
        pendingGeneric = null;
        continue;
      }
    }

    // "Valor: 264,00 UA/mL" / "Índice: 0,07" / "S/CO da amostra: 0,06" logo
    // após um qualitativo genérico: anexa o número ao resultado
    if (lastGeneric) {
      const am = line
        .trim()
        .match(/^(valor|[íi]ndice|s\/?\s?co(\s+da\s+amostra)?)\s*:?\s*([<>]?\s*-?[\d.,]+)/);
      if (am) {
        lastGeneric.r.raw += ' ' + am[3].replace(/\s+/g, '').replace('.', ',');
        lastGeneric = null;
        continue;
      }
      if (--lastGeneric.ttl <= 0) lastGeneric = null;
    }
    ...
    // ---- valor de relação/componente urinário pendente (fluxo normal)
    if (pendingRatio && VALUE_LINE.test(line.trim()) && !RANGE_LINE.test(line)) {
      const v = findValue(line.trim(), 0, {});
      if (v) {
        const intDigits = v.tok.replace(',', '.').split('.')[0].replace('-', '').length;
        const bare = /^\s*(resultado\s*(\([^)]*\))?\s*:*\s*)?[<>]?\s*-?[\d.,]+\s*$/.test(line.trim());
        if (intDigits <= 6 && (bare || UNIT_AFTER.test(v.after || ''))) {
          pushResult(pendingRatio.a, v, currentDTNow());
          pendingRatio = null;
          continue;
  ...
  // traz a relação pronta (mesma liberação; valores já em mg/dL)
  {
    ...
    for (const [, g] of bySort) {
      // ânion gap urinário: (Na + K) − Cl da mesma amostra (não precisa de Cr)
      const nau = g.get('U-NaQ');
      const ku = g.get('U-KQ');
      const clu = g.get('U-ClQ');
      if (nau && ku && clu && !g.has('AGU')) {
        const v = nau.value + ku.value - clu.value;
        results.push({
          id: 'AGU', label: 'AGu', value: v,
          raw: (v >= 0 ? '+' : '') + fmtNum(Math.round(v * 10) / 10),
          pct: false, dt: nau.dt,
        });
      }
      ...
      const cau = g.get('U-CaQ');
      if (cau && !g.has('CACR')) {
        const v = cau.value / cr.value; // calciúria: mg/mg
        results.push({
          id: 'CACR', label: 'Ca/Cr', value: v,
          raw: v.toFixed(2).replace('.', ','), pct: false, dt,
        });
      }
    }
