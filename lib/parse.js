...
const QUALITATIVE =
  /^(nao\s+)?(reagente|detectado|positivo|negativo|ausente|presente|indetectavel|inconclusivo|reator)/;

// linhas que nunca são título de exame genérico
const GENERIC_SKIP =
  /(hemograma|eritrograma|leucograma|contagem diferencial|gasometria|coagulograma|plaquetas|resultado|urina\b|\beas\b|observa|\bnota\b|adendo|assinatura|conferido|repetido|confirmado|automatizado)/;
...
function genericTitle(rawCut, normCut) {
  ...
  if (upper.length / letters.length < 0.7) return null; // títulos vêm em CAPS
...
export function parseReport(text, opts = {}) {
  ...
  function urineLine(nline, rline) {
    ...
    for (const a of COMPILED) {
      ...
      for (const { rx } of a.compiled) {
        ...
        else pendingRatio = { a, ttl: 8 };
        return;
      }
    }
    if (pendingRatio && VALUE_LINE.test(nline.trim())) {
  ...
  for (const rawLine of rawLines) {
    ...
    if (URINE_SECTION.test(norm)) {
      ...
      // título de relação P/C ou M/C na própria linha da seção
    ...
    if (pendingGeneric && !REF_WRAP.test(line)) {
      const nt = line.trim();
      const t = rawCut.trim();
      const rm = nt.match(/^resultado[^:]{0,14}:*\s*/);
      ...
      } else if ((rm || QUALITATIVE.test(restNorm)) && restNorm) {
    ...
    if (valuesOnLine === 0 && nameOnly.size === 1) {
      ...
      pendingGeneric = null;
    } else if (valuesOnLine === 0 && nameOnly.size === 0 && !pending && !pendingGeneric) {
