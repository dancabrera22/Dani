...
export const IGNORE_PATTERNS = [
  ...
  '\\bblastos?\\b',
  'outras celulas',
  // antígeno P24 do HIV nunca é fósforo
  'p\\s?24',
...
export const URINE_SECTION =
  /(urina\s+tipo|urina\s+rotina|\beas\b|urinalise|sumario\s+de\s+urina|urina\s+i\b|urina\s+24|urocultura|exame\s+de\s+urina|urina,?\s+exame|elementos\s+anormais|urina\s+(isolada|amostra)|amostra\s+isolada|sedimento\s+urinario)/;
...
export const ANALYTES = [
  ...
  // componentes urinários (ocultos): usados para CALCULAR P/C e M/C quando o
  // laudo não traz a relação pronta. Vêm ANTES de Cr/Alb/PT para mascarar.
  { id: 'CACR',    label: 'Ca/Cr',  patterns: ['(relacao\\s+)?calcio\\s*\\/\\s*creatinina', 'calciuria\\s*\\/\\s*creatininuria'] },
  { id: 'U-CaQ',   label: 'CaU',    hidden: true, patterns: ['calcio\\s+urin[aá]rio', 'calcio\\s*[,(]\\s*urina', '\\bcalciuria\\b'] },
  { id: 'U-NaQ',   label: 'NaU',    hidden: true, patterns: ['sodio\\s+urin[aá]rio', 'sodio\\s*[,(]\\s*urina'] },
  { id: 'U-KQ',    label: 'KU',     hidden: true, patterns: ['potassio\\s+urin[aá]rio', 'potassio\\s*[,(]\\s*urina'] },
  { id: 'U-ClQ',   label: 'ClU',    hidden: true, patterns: ['cloro\\s+urin[aá]rio', 'cloro\\s*[,(]\\s*urina', 'cloreto\\s*[,(]?\\s*urin[aá]?'] },
  { id: 'U-PQ',    label: 'PU',     hidden: true, patterns: ['fosforo\\s+urin[aá]rio', 'fosforo\\s*[,(]\\s*urina', 'fosfaturia'] },
  { id: 'U-OsmQ',  label: 'OsmU',   hidden: true, patterns: ['osmolalidade\\s+urin[aá]ria', 'osmolalidade\\s*[,(]\\s*urina', 'osmolaridade\\s+urin[aá]ria', 'osmolaridade\\s*[,(]\\s*urina'] },
  { id: 'U-UrQ',   label: 'UrU',    hidden: true, patterns: ['ureia\\s+urin[aá]ria', 'ureia\\s*[,(]\\s*urina', 'ureia\\s+na\\s+urina'] },
  { id: 'U-MgQ',   label: 'MgU',    hidden: true, patterns: ['magnesio\\s+urin[aá]rio', 'magnesio\\s*[,(]\\s*urina', 'magnesuria'] },
  { id: 'U-AUQ',   label: 'AUu',    hidden: true, patterns: ['acido\\s+urico\\s+urin[aá]rio', 'acido\\s+urico\\s*[,(]\\s*urina', 'uricosuria'] },
  { id: 'U-GlicQ', label: 'GlicU',  hidden: true, patterns: ['glicose\\s+urin[aá]ria', 'glicose\\s*[,(]\\s*urina', 'glicosuria\\s+quantitativa'] },
  { id: 'U-VolQ',  label: 'VU',     hidden: true, patterns: ['volume\\s+urin[aá]rio', 'volume\\s+(total\\s+)?(de\\s+)?24', 'diurese(\\s+de\\s+24\\s*h(oras)?)?', 'volume\\s+da\\s+diurese'] },
  { id: 'ClCr',    label: 'ClCr',   patterns: ['clearance\\s+de\\s+creatinina', 'depura[çc][ãa]o\\s+de\\s+creatinina'] },
  { id: 'Osm',     label: 'Osm',    patterns: ['osmolalidade(\\s+serica)?', 'osmolaridade(\\s+serica)?'] },
  // imunoglobulinas séricas (padrões restritos a dosagem — não contaminam
  // sorologias "Toxoplasmose IgG")
  { id: 'IgG',   label: 'IgG',     patterns: ['imunoglobulina\\s+g\\b', 'igg[\\s,]*\\(?\\s*(soro|sangue|serica)', 'dosagem\\s+de\\s+igg'] },
  { id: 'IgA',   label: 'IgA',     patterns: ['imunoglobulina\\s+a\\b', 'iga[\\s,]*\\(?\\s*(soro|sangue|serica)', 'dosagem\\s+de\\s+iga'] },
  { id: 'IgM',   label: 'IgM',     patterns: ['imunoglobulina\\s+m\\b', 'igm[\\s,]*\\(?\\s*(soro|sangue|serica)', 'dosagem\\s+de\\s+igm'] },
  { id: 'IgE',   label: 'IgE',     patterns: ['imunoglobulina\\s+e\\b', 'ige[\\s,]*\\(?\\s*(soro|sangue|serica)', 'ige\\s+total', 'dosagem\\s+de\\s+ige'] },
  { id: 'U-CrQ',   label: 'CreatU', hidden: true, patterns: ['creatinina\\s+urin[aá]ria', 'creatinina\\s*[,(]\\s*urina', 'creatininuria'] },
  { id: 'U-AlbQ',  label: 'AlbU',   hidden: true, patterns: ['(micro)?albumina\\s+urin[aá]ria', 'microalbuminuria'] },
  { id: 'U-ProtQ', label: 'ProtU',  hidden: true, patterns: ['prote[ií]nas?\\s+urin[aá]rias?', 'prote[ií]nas?\\s+(tota(l|is)\\s*)?[,(]\\s*urina', 'proteinuria\\s+(isolada|de\\s+amostra)', '\\bproteinuria\\b'] },
  ...
  { id: 'Trig',  label: 'TG',      patterns: ['triglicerid[a-z]*', '\\btg\\b'] },
  ...
  { id: 'FK',    label: 'NS FK',   patterns: ['tacrolimus', 'tacrolimo', 'fk\\s?-?506', '\\bfk\\b'] },
  ...
  { id: 'AntiXa', label: 'Anti-Xa', patterns: ['atividade\\s+anti[\\s-]?xa', 'anti[\\s-]?fator\\s+xa', 'fator\\s+anti[\\s-]?xa', 'anti[\\s-]?xa'] },
...
export const EXTRA_ORDER = [
  ...
  'TP', 'INR', 'TTPa', 'Fibr', 'AntiXa', 'FK', 'Osm',
  'IgG', 'IgA', 'IgM', 'IgE',
];
