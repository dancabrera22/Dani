...
// ânion gap sérico: Na − (Cl + HCO3); com albumina baixa, corrigido
// (AGc = AG + 2,5 × (4 − Alb))
function renderAG(byId) {
  ...
  let out = `AG ${fmtNum(Math.round(ag * 10) / 10)}`;
  const alb = byId.get('Alb');
  if (alb && alb.value < 4) {
    const agc = ag + 2.5 * (4 - alb.value);
    out += ` (corr ${fmtNum(Math.round(agc * 10) / 10)})`;
  }
  return out;
}
