...
// ---- diálise por dia: R, Kt/V da sessão e semanal (a partir das ureias
// pré/pós do laudo + tempo, pesos e sessões digitados)
function hdByDay(results, opts = {}) {
  const map = new Map();
  const byDay = new Map();
  for (const r of results) {
    if (r.id !== 'Ur' || !r.dt) continue;
    if (!byDay.has(r.dt.dayKey)) byDay.set(r.dt.dayKey, []);
    byDay.get(r.dt.dayKey).push(r);
  }
  for (const [day, arr] of byDay) {
    ...
    if (!(pre.value > 0)) continue;
    const R = pos.value / pre.value;
    const info = {
      preRaw: pre.raw, posRaw: pos.raw, R,
      urr: ((pre.value - pos.value) / pre.value) * 100,
      sp: null, std: null,
    };
    const tmin = Number(opts.hdTime);
    const wPre = Number(opts.hdPre);
    const wPos = Number(opts.hdPos);
    const w = wPos > 0 ? wPos : Number(opts.weight);
    if (tmin > 0 && w > 0) {
      const th = tmin / 60;
      const ufL = wPre > 0 && wPos > 0 && wPre > wPos ? wPre - wPos : 0;
      const sp = -Math.log(R - 0.008 * th) + (4 - 3.5 * R) * (ufL / w);
      if (Number.isFinite(sp) && sp > 0) {
        Object.assign(info, { sp, th, ufL, w, tmin });
        const e = sp * (tmin / (tmin + 35));
        const N = Number(opts.hdSessions) > 0 ? Number(opts.hdSessions) : 3;
        const x = 1 - Math.exp(-e);
        const std = ((10080 * x) / tmin) / (x / e + 10080 / (N * tmin) - 1);
        if (Number.isFinite(std) && std > 0) Object.assign(info, { e, N, std });
      }
    }
    map.set(day, info);
  }
...
function agLines(results, opts = {}) {
  ...
  for (const { dt, byId } of buildBlocks(results)) {
    ...
    // ---- diálise: contas armadas (os valores limpos ficam no MENSALÃO).
    // Uma vez por DIA, não por coleta (pré e pós são blocos diferentes).
    if (opts.hd && opts.hd.has(dt.dayKey) && !hdDone.has(dt.dayKey)) {
      hdDone.add(dt.dayKey);
      const h = opts.hd.get(dt.dayKey);
      if (h.urr != null) {
        segs.push(`URR (${h.preRaw}-${h.posRaw})/${h.preRaw} = ${f1(h.urr)}%`);
      }
      if (h.sp != null) {
        const f2 = (v) => fmtNum(Math.round(v * 100) / 100);
        segs.push(
          `Kt/V -ln(${f2(h.R)}-0,008x${f2(h.th)})+(4-3,5x${f2(h.R)})x${f2(h.ufL)}/${fmtNum(h.w)} = ${f2(h.sp)}`
        );
        if (h.std != null) {
          segs.push(
            `Kt/V semanal (eKt/V ${f2(h.e)}, ${h.N}x/sem, ${fmtNum(h.tmin)} min) = ${f2(h.std)}`
          );
        }
      }
    }
...
export function formatTubulopatias(results, opts = {}) {
  ...
  const withHd = { ...opts, hd: hdByDay(results, opts) };
  const ags = new Map(agLines(results, withHd).map((a) => [a.sort, a.line]));
  for (const { dt, byId } of buildBlocks(results)) {
  ...
  for (const lineIds of MENSALAO_LINES) {
    ...
    if (lineIds[0] === 'Ur' && lineIds[1] === 'Cr') {
      for (const { dt } of days) {
        ...
        // diálise: valores limpos numa linha (URR, Kt/V da sessão e semanal)
        const hdParts = [];
        if (urs.length > 1 && urs[0].value > 0) {
          const last = urs[urs.length - 1];
          const urr = ((urs[0].value - last.value) / urs[0].value) * 100;
          hdParts.push(`URR ${fmtNum(Math.round(urr * 10) / 10)}%`);
        }
        // Kt/V (Daugirdas 2ª geração) + semanal (stdKt/V, FHN/Daugirdas)
        if (urs.length > 1 && urs[0].value > 0) {
          const R = urs[urs.length - 1].value / urs[0].value;
          const tmin = Number(opts.hdTime);
          const pre = Number(opts.hdPre);
          const pos = Number(opts.hdPos);
          // peso pós-diálise é o peso do modelo; UF = pré − pós
          const w = pos > 0 ? pos : Number(opts.weight);
          if (tmin > 0 && w > 0) {
            const th = tmin / 60;
            const ufL = pre > 0 && pos > 0 && pre > pos ? pre - pos : 0;
            const sp = -Math.log(R - 0.008 * th) + (4 - 3.5 * R) * (ufL / w);
            if (Number.isFinite(sp) && sp > 0) {
              const f2 = (v) => fmtNum(Math.round(v * 100) / 100);
              hdParts.push(`Kt/V ${f2(sp)}`);
              const e = sp * (tmin / (tmin + 35)); // eKt/V (Tattersall, acesso AV)
              const N = Number(opts.hdSessions) > 0 ? Number(opts.hdSessions) : 3;
              const x = 1 - Math.exp(-e);
              const std = ((10080 * x) / tmin) / (x / e + 10080 / (N * tmin) - 1);
              if (Number.isFinite(std) && std > 0) {
                hdParts.push(`Kt/V semanal ${f2(std)}`);
              }
            }
          }
        }
        if (hdParts.length) lines.push(`${dt.date}: ${hdParts.join(' ')}`);
