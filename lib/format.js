...
function tubRatios(byId) {
  const parts = [];
  for (const [id, label] of [['PC', 'P/C'], ['MC', 'M/C'], ['CACR', 'Ca/Cr']]) {
    const r = byId.get(id);
...
function agLines(results) {
  ...
  for (const { dt, byId } of buildBlocks(results)) {
    ...
    if (segs.length) out.push({ sort: dt.sortKey, line: `${dt.date}: ${segs.join(' // ')}` });
  }
  return out;
}
...
function renderBlock(dt, byId, { withQuarter = false, withRatio = false } = {}) {
  const parts = [];
...
export function renderTubulopatias(results) {
  const lines = [];
  let hasUrinary = false;
  const ags = new Map(agLines(results).map((a) => [a.sort, a.line]));
  for (const { dt, byId } of buildBlocks(results)) {
    ...
    const ag = ags.get(dt.sortKey);
    if (ag && (comps || ratios || byId.has('AGU'))) lines.push(ag);
  }
  if (!hasUrinary || lines.length === 0) return null;
  return 'TUBULOPATIAS:\n' + lines.join('\n');
}
...
export function formatRotina(results) {
  const blocks = [];
  for (const { dt, byId } of buildBlocks(results)) {
    const b = renderBlock(dt, byId);
    if (b) blocks.push(b);
  }
  const tub = renderTubulopatias(results);
  const generics = renderGenerics(results, !tub);
  if (generics) blocks.push(generics);
  if (tub) blocks.push(tub);
  return blocks.join('\n\n');
}
...
export function formatPrisma(results) {
  const blocks = [];
  for (const { dt, byId } of buildBlocks(results)) {
    const b = renderBlock(dt, byId, { withQuarter: true, withRatio: true });
    if (b) blocks.push(b);
  }
  const tub = renderTubulopatias(results);
  const generics = renderGenerics(results, !tub);
