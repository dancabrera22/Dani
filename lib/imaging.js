...
export function parseImagingConclusions(text) {
  ...
  // fallback: laudo SEM seção de conclusão (achados direto na ANÁLISE) —
  // transcreve o início da análise, com aviso
  if (items.length === 0) {
    const START = /^\s*(an[áa]lise|achados|relat[óo]rio)\s*[:.]?\s*$/i;
    for (let idx = 0; idx < lines.length; idx++) {
      if (!START.test(lines[idx])) continue;
      const buf = [];
      let blanks = 0;
      for (let j = idx + 1; j < lines.length && j < idx + 14; j++) {
        ...
        blanks = 0;
        if (TERMINATOR.test(n2) || NEXT_SECTION.test(n2) || CONCLUSION_HEAD.test(r2)) break;
        buf.push(r2.trim());
        if (buf.join(' ').length > 600) break;
      }
      const out = buf.join(' ').replace(/\s+/g, ' ').trim().slice(0, 650);
      if (out) {
        items.push({ dt: (blockBest || lastAny)?.dt ?? null, text: out });
        warnings.push(
          'Laudo de imagem SEM seção de conclusão — transcrito o início da ANÁLISE/ACHADOS. Confira o laudo completo.'
        );
      }
      break;
    }
